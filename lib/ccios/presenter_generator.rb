...
class PresenterGenerator
  ...
  def generate(presenter_name, options = {})
    ...
    if @parser.presenter_path
      new_group_path = File.join(@parser.presenter_path, presenter_name)
      ...
      file_creator.create_file_at_path(presenter_name, 'ViewContract', ui_group_path, target)
      file_creator.create_file_at_path(presenter_name, 'ViewController', view_controller_group_path, target)
      file_creator.create_file_at_path(presenter_name, 'Presenter', presenter_group_path, target)
      file_creator.create_file_at_path(presenter_name, 'PresenterImplementation', presenter_group_path, target)
      file_creator.create_empty_directory_at_path(model_group_path)
      file_creator.create_empty_directory_at_path(view_group_path)
    else
